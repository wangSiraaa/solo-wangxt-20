...
export type SubmitResult =
  | { status: 'merged'; revision: number; submissionId: string; deduplicated?: boolean }
  | {
      status: 'conflict';
      conflictId: string;
      submissionId: string;
...
export class SegmentsService {
  ...
  async list(projectId: string, language: string, versionNo?: number) {
    const version = versionNo
      ? (
          await this.db.query(
            `SELECT * FROM picture_versions WHERE project_id=$1 AND version_no=$2`,
            [projectId, versionNo],
          )
        )[0]
      : (
    ...
    const rows = await this.db.query(
      `SELECT s.id, s.seg_no, s.source_text, s.source_revision, s.cross_shot,
  ...
  async getSegment(id: string) {
    ...
    if (!rows[0]) throw new Error(`片段不存在: ${id}`);
    return rows[0];
  }

  async claim(segmentId: string, language: string, user: string) {
    await this.getSegment(segmentId);
    const active = await this.db.query(
      `SELECT * FROM assignments WHERE segment_id=$1 AND language=$2 AND status='claimed'`,
      [segmentId, language],
    );
    if (active[0]) {
      if (active[0].assignee === user) return active[0];
      const err: any = new Error(`片段已被 ${active[0].assignee} 领取`);
    ...
    const id = uuid();
    await this.db.query(
      `INSERT INTO assignments (id, segment_id, language, assignee, status, claimed_at)
       VALUES ($1,$2,$3,$4,'claimed',$5)`,
      [id, segmentId, language, user, nowIso()],
    );
    return { id, segmentId, language, assignee: user };
  }

  async release(segmentId: string, language: string, user: string) {
    await this.db.query(
      `UPDATE assignments SET status='released'
       WHERE segment_id=$1 AND language=$2 AND status='claimed' AND assignee=$3`,
      [segmentId, language, user],
    );
  }
    ...
    return this.db.tx(async (db) => {
      ...
      if (idempotencyKey) {
        const dup = await db.query(
          `SELECT * FROM submissions WHERE idempotency_key=$1`,
          [idempotencyKey],
        );
        if (dup[0]) {
          const s = dup[0];
          if (s.status === 'merged') {
            const cur = await db.query(
              `SELECT revision FROM segment_texts WHERE segment_id=$1 AND language=$2`,
              [segmentId, language],
            );
            return {
              status: 'merged',
          ...
          const c = await db.query(
            `SELECT * FROM conflicts WHERE submission_id=$1 ORDER BY created_at DESC LIMIT 1`,
            [s.id],
          );
          const cur = await db.query(
            `SELECT * FROM segment_texts WHERE segment_id=$1 AND language=$2`,
            [segmentId, language],
          );
          return {
            status: 'conflict',
      ...
      if (baseRevision === currentRevision) {
        ...
        await db.query(
          `INSERT INTO segment_texts (segment_id, language, text, revision, updated_by, updated_at)
           ...
           DO UPDATE SET text=$3, revision=$4, updated_by=$5, updated_at=$6`,
          [segmentId, language, text, newRevision, author, nowIso()],
        );
        await db.query(
          `INSERT INTO submissions (id, segment_id, language, author, text, base_revision, status, idempotency_key, created_at)
           VALUES ($1,$2,$3,$4,$5,$6,'merged',$7,$8)`,
          [submissionId, segmentId, language, author, text, baseRevision, idempotencyKey ?? null, nowIso()],
        );
      ...
      await db.query(
        `INSERT INTO submissions (id, segment_id, language, author, text, base_revision, status, idempotency_key, created_at)
         VALUES ($1,$2,$3,$4,$5,$6,'conflicted',$7,$8)`,
        [submissionId, segmentId, language, author, text, baseRevision, idempotencyKey ?? null, nowIso()],
      );
      const conflictId = uuid();
      await db.query(
        `INSERT INTO conflicts (id, segment_id, language, submission_id, current_revision, status, created_at)
         VALUES ($1,$2,$3,$4,$5,'open',$6)`,
        [conflictId, segmentId, language, submissionId, currentRevision, nowIso()],
      );
      return {
        status: 'conflict',
        conflictId,
  ...
  private async autoTerminologyCheck(
    db: Db,
    segmentId: string,
    language: string,
    text: string,
    ...
    const terms = await db.query(
      `SELECT * FROM glossary_terms WHERE project_id=$1 AND language=$2`,
      [seg[0].project_id, language],
    );
    for (const term of terms) {
      if (
        seg[0].source_text.includes(term.source) &&
        ...
        const existing = await db.query(
          `SELECT id FROM review_issues
           WHERE segment_id=$1 AND language=$2 AND type='terminology' AND status='open' AND note=$3`,
          [segmentId, language, note],
        );
        if (!existing[0]) {
          await db.query(
            `INSERT INTO review_issues (id, segment_id, language, type, note, status, created_by, created_at)
             VALUES ($1,$2,$3,'terminology',$4,'open',$5,$6)`,
            [uuid(), segmentId, language, note, `自动检查(${author})`, nowIso()],
          );
        }
      }
    }
  ...
  async listConflicts(projectId: string, status = 'open') {
    return this.db.query(
      `SELECT c.*, s.seg_no, sub.text AS submitted_text, sub.author AS submitted_by,
  ...
  async resolveConflict(
    conflictId: string,
    strategy: 'take_mine' | 'take_current' | 'merge',
    ...
    return this.db.tx(async (db) => {
      ...
      if (conflict.status !== 'open') {
        const err: any = new Error('冲突已被解决，请刷新查看最新状态');
        err.code = 'ALREADY_RESOLVED';
        throw err;
      }
      const sub = (
        await db.query(`SELECT * FROM submissions WHERE id=$1`, [conflict.submission_id])
      )[0];
      const cur = (
        await db.query(
          `SELECT * FROM segment_texts WHERE segment_id=$1 AND language=$2`,
          [conflict.segment_id, conflict.language],
        )
      )[0] ?? null;
      ...
      if (strategy === 'take_current') {
        ...
        if (!finalText) throw new Error('合并策略需要提供 mergedText');
        await db.query(
          `INSERT INTO segment_texts (segment_id, language, text, revision, updated_by, updated_at)
           VALUES ($1,$2,$3,$4,$5,$6)
           ON CONFLICT (segment_id, language)
           DO UPDATE SET text=$3, revision=$4, updated_by=$5, updated_at=$6`,
          [conflict.segment_id, conflict.language, finalText, currentRevision + 1, user, nowIso()],
        );
        await db.query(`UPDATE submissions SET status='merged' WHERE id=$1`, [sub.id]);
        await this.autoTerminologyCheck(
          db,
          conflict.segment_id,
          conflict.language,
          finalText,
      ...
      await db.query(
        `UPDATE conflicts SET status='resolved', resolution=$1, resolved_by=$2, resolved_at=$3 WHERE id=$4`,
        [strategy, user, nowIso(), conflictId],
      );
      // 同一片段语言的其他未决冲突：基准已过期，标记为 superseded
      const others = await db.query(
        `SELECT * FROM conflicts WHERE segment_id=$1 AND language=$2 AND status='open' AND id<>$3`,
        [conflict.segment_id, conflict.language, conflictId],
      );
      for (const o of others) {
        await db.query(
          `UPDATE conflicts SET status='resolved', resolution='superseded', resolved_by=$1, resolved_at=$2 WHERE id=$3`,
          [user, nowIso(), o.id],
        );
  ...
  async retime(
    ...
    const timing = (
      await this.db.query(
        ...
        [segmentId, version.id],
      )
    )[0];
    if (!timing) throw new Error('当前版本没有该片段的时间轴');
    await this.db.query(
      `UPDATE segment_timings
